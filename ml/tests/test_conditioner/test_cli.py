...
def test_build_conditioner(monkeypatch, artifacts_path):
    ...
    monkeypatch.setattr("moomoo_ml.conditioner.cli.fetch_numpy_embeddings", lambda: (paths, embeds))
...
def test_condition_new_files(monkeypatch):
    ...
    with get_session() as session:
        for i in range(50):
            ...
            session.add(
                FileEmbedding(filepath=Path(f"{i}.mp3"), success=True, embedding=vec.tolist())
            )
        session.commit()
    ...
    monkeypatch.setattr("moomoo_ml.conditioner.cli.fetch_numpy_embeddings", fake_result(2000))
    result = runner.invoke(build_conditioner, ["--update-info"])
    assert result.exit_code == 0

    # no data
    monkeypatch.setattr(
        "moomoo_ml.conditioner.cli.fetch_numpy_embeddings", lambda **_: [[], np.array([])]
    )
    ...
    # condition the files
    monkeypatch.setattr("moomoo_ml.conditioner.cli.fetch_numpy_embeddings", fake_result(10))
    result = runner.invoke(condition_new_files)
    assert result.exit_code == 0
    ...
    with get_session() as session:
        ...
        condition = lambda r: int(str(r.filepath).split(".")[0]) < 10  # noqa: E731
