...
from moomoo.db import ListenBrainzListen, execute_sql_fetchall
from moomoo.ingest import collect_listen_data
from moomoo.utils_ import utcnow
...
def test_cli_main__not_table_exists_error():
    ...
    name = ListenBrainzListen.table_name()
...
def test_cli_main__since_last__date_buffer():
    ...
    with patch("moomoo.ingest.collect_listen_data.run_ingest") as mocked:
        result = runner.invoke(
...
def test_run_ingest__insert_upsert():
    ListenBrainzListen.create()
    sql = f"select count(1) as n from {ListenBrainzListen.full_name()}"
