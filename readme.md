...
- [`client`](client/) provides an installable (via pipx, etc) package for local clients. Its requirements are minimal and should be lightweight. It has commands to generate playlists which can be exported to text files and strawberry music player. It also has an experimental/poorly maintained GUI which can be used to load up the aforementioned playlists.
- [`navidrome`](navidrome/) is an integration layer between moomoo's internals, listenbrainz, and a [navidrome](https://www.navidrome.org/) server. It contains commands to load playlists into navidrome for streaming use.
